...
def bandit27(view_output=False):
    ...
    _, _, _ = ssh.exec_command(F'rm -rf {path + "/repo"}')
    channel = ssh.get_transport().open_session()
    channel.get_pty()
    channel.invoke_shell()
    time.sleep(1)

    clone_command = 'git clone ssh://bandit27-git@localhost/home/bandit27-git/repo'
    channel.send(F'cd {path} && {clone_command} \n')
    max_bytes = 3072
    while channel.recv_ready():
        time.sleep(1)
        output = channel.recv(max_bytes).decode()
        if view_output:
            print(output)
    ...
    channel.send(credentials['bandit27'] + '\n')
    time.sleep(1)
    while channel.recv_ready():
        time.sleep(1)
        output = channel.recv(max_bytes).decode()
    ...
    _, out, _ = ssh.exec_command(F'cat {path + "/repo/README"} | cut -d " " -f8')
    passwd = out.readline().strip()
...
* Bandit28 => Bandit29
#+begin_src python :tangle ape.py :session :results silent
def bandit28(view_output=False):
    ...
    clone_command = 'git clone ssh://bandit28-git@localhost/home/bandit28-git/repo'
    channel.send(F'cd {path} && {clone_command} \n')
    max_bytes = 3072
    while channel.recv_ready():
        time.sleep(1)
    ...
    channel.send(credentials['bandit28'] + '\n')
    time.sleep(1)
    while channel.recv_ready():
        time.sleep(1)
        output = channel.recv(max_bytes).decode()
    ...
    command = F'cd {path + "/repo"} && git log | grep -E \'^commit\' | cut -d " " -f2 | while read line; do git show $line; done | grep -E \'\\+- password: [^x<]\' | cut -d " " -f3'
    _, out, _ = ssh.exec_command(command)
    passwd = out.readline().strip()
